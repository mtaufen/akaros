                           ...
                           uint32_t size, char *file, uint32_t line)
{
	...
	//       around. We can probably also check that the pointer isn't outside
	...
	do {
		...
		if (*olen + *ilen > max) {
			VIRTIO_DRI_ERRX(vq->vqdev,
				...
				"\n  See virtio-v1.0-cs04 s2.4.5.2 The Virtqueue"
				" Descriptor Table");
		}
...
const char *virtio_validate_feat(struct virtio_vq_dev *vqdev, uint64_t feat) {
	...
	if (!(feat & ((uint64_t)1<<VIRTIO_F_VERSION_1)))
		return "A device MUST offer the VIRTIO_F_VERSION_1 feature bit and"
		       " a driver MUST accept it."
