...
void baseline_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("baseline_xsave", 1, "xsave64", 1, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("baseline_xsave", 1, "xrstor64", 2, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("baseline_xsaveopt", 1, "xsaveopt64", 3, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("baseline_xsaveopt", 1, "xrstor64", 4, end - start);
	}
}
...
void x87_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsave", 2, "xsave64", 5, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsave", 2, "xrstor64", 6, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsaveopt", 2, "xsaveopt64", 7, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsaveopt", 2, "xrstor64", 8, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsave", 3, "xsave64", 9, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsave", 3, "xrstor64", 10, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsaveopt", 3, "xsaveopt64", 11, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsaveopt", 3, "xrstor64", 12, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsave", 4, "xsave64", 13, end - start);
		fninit();
		dirty_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsave", 4, "xrstor64", 14, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("x87_xsaveopt", 4, "xsaveopt64", 15, end - start);
		fninit();
		dirty_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("x87_xsaveopt", 4, "xrstor64", 16, end - start);
	}
}
...
void xmm_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsave", 2, "xsave64", 17, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsave", 2, "xrstor64", 18, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsaveopt", 2, "xsaveopt64", 19, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsaveopt", 2, "xrstor64", 20, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsave", 3, "xsave64", 21, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsave", 3, "xrstor64", 22, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsaveopt", 3, "xsaveopt64", 23, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsaveopt", 3, "xrstor64", 24, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsave", 4, "xsave64", 25, end - start);
		fninit();
		dirty_xmm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsave", 4, "xrstor64", 26, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_xsaveopt", 4, "xsaveopt64", 27, end - start);
		fninit();
		dirty_xmm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_xsaveopt", 4, "xrstor64", 28, end - start);
	}
}
...
void hi_ymm_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsave", 2, "xsave64", 29, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsave", 2, "xrstor64", 30, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsaveopt", 2, "xsaveopt64", 31, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsaveopt", 2, "xrstor64", 32, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsave", 3, "xsave64", 33, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsave", 3, "xrstor64", 34, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsaveopt", 3, "xsaveopt64", 35, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsaveopt", 3, "xrstor64", 36, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsave", 4, "xsave64", 37, end - start);
		fninit();
		dirty_hi_ymm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsave", 4, "xrstor64", 38, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xsaveopt", 4, "xsaveopt64", 39, end - start);
		fninit();
		dirty_hi_ymm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xsaveopt", 4, "xrstor64", 40, end - start);
	}
}
...
void xmm_x87_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsave", 2, "xsave64", 41, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsave", 2, "xrstor64", 42, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsaveopt", 2, "xsaveopt64", 43, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsaveopt", 2, "xrstor64", 44, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsave", 3, "xsave64", 45, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsave", 3, "xrstor64", 46, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsaveopt", 3, "xsaveopt64", 47, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsaveopt", 3, "xrstor64", 48, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsave", 4, "xsave64", 49, end - start);
		fninit();
		dirty_xmm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsave", 4, "xrstor64", 50, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("xmm_x87_xsaveopt", 4, "xsaveopt64", 51, end - start);
		fninit();
		dirty_xmm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("xmm_x87_xsaveopt", 4, "xrstor64", 52, end - start);
	}
}
...
void hi_ymm_xmm_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsave", 2, "xsave64", 53, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsave", 2, "xrstor64", 54, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsaveopt", 2, "xsaveopt64", 55, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsaveopt", 2, "xrstor64", 56, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsave", 3, "xsave64", 57, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsave", 3, "xrstor64", 58, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsaveopt", 3, "xsaveopt64", 59, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsaveopt", 3, "xrstor64", 60, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsave", 4, "xsave64", 61, end - start);
		fninit();
		dirty_hi_ymm_xmm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsave", 4, "xrstor64", 62, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_xsaveopt", 4, "xsaveopt64", 63, end - start);
		fninit();
		dirty_hi_ymm_xmm();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_xsaveopt", 4, "xrstor64", 64, end - start);
	}
}
...
void hi_ymm_x87_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsave", 2, "xsave64", 65, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsave", 2, "xrstor64", 66, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsaveopt", 2, "xsaveopt64", 67, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsaveopt", 2, "xrstor64", 68, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsave", 3, "xsave64", 69, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsave", 3, "xrstor64", 70, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsaveopt", 3, "xsaveopt64", 71, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsaveopt", 3, "xrstor64", 72, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsave", 4, "xsave64", 73, end - start);
		fninit();
		dirty_hi_ymm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsave", 4, "xrstor64", 74, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_x87_xsaveopt", 4, "xsaveopt64", 75, end - start);
		fninit();
		dirty_hi_ymm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_x87_xsaveopt", 4, "xrstor64", 76, end - start);
	}
}
...
void hi_ymm_xmm_x87_tests(uint64_t n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsave", 2, "xsave64", 77, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsave", 2, "xrstor64", 78, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsaveopt", 2, "xsaveopt64", 79, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsaveopt", 2, "xrstor64", 80, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsave", 3, "xsave64", 81, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsave", 3, "xrstor64", 82, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsaveopt", 3, "xsaveopt64", 83, end - start);
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsaveopt", 3, "xrstor64", 84, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsave", 4, "xsave64", 85, end - start);
		fninit();
		dirty_hi_ymm_xmm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsave", 4, "xrstor64", 86, end - start);
	}
	...
	for (i = 0; i < n; ++i) {
		...
		print_results("hi_ymm_xmm_x87_xsaveopt", 4, "xsaveopt64", 87, end - start);
		fninit();
		dirty_hi_ymm_xmm_x87();
		start = readTSC();
		xrstor64(&as);
		end = readTSC();
		print_results("hi_ymm_xmm_x87_xsaveopt", 4, "xrstor64", 88, end - start);
	}
}
