...
void ancillary_state_init()
{
	...
	*/
	...
	/*
		...
		We need to use FNINIT to reset the FPU before saving, in case boot agents
		used the FPU or it is dirty for some reason. An old comment that used to be
		here said "had this happen on c89, which had a full FP stack after booting."
		Note that FNINIT does not clear the data registers, but it tags them
		all as empty (0b11).
	*/
	asm volatile ("fninit");
